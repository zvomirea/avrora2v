...
(function()
{
	...
	// Cycle-bgin shape
	function CycleBeginShape()
	{
		mxActor.call(this);
	};
	mxUtils.extend(CycleBeginShape, mxActor);
	CycleBeginShape.prototype.size = 30;
	CycleBeginShape.prototype.width = 60;
	CycleBeginShape.prototype.height = 30;
	CycleBeginShape.prototype.redrawPath = function(c, x, y, w, h)
	{
		var s = Math.max(0, Math.min(w, Math.min(h, parseFloat(mxUtils.getValue(this.style, 'size', this.size)))) / 2);
		var arcSize = mxUtils.getValue(this.style, mxConstants.STYLE_ARCSIZE, mxConstants.LINE_ARCSIZE) / 2;
		this.addPoints(c, [new mxPoint(s, 0), new mxPoint(w - s, 0), new mxPoint(w, s), new mxPoint(w, h), new mxPoint(0, h), new mxPoint(0, s)],
				this.isRounded, arcSize, true);
		c.end();
	};

	mxCellRenderer.registerShape('cycle_begin', CycleBeginShape);
	
	// Cycle-bgin shape
	function CycleEndShape()
	{
		mxActor.call(this);
	};
	mxUtils.extend(CycleEndShape, mxActor);
	CycleEndShape.prototype.size = 30;
	CycleEndShape.prototype.width = 60;
	CycleEndShape.prototype.height = 30;
	CycleEndShape.prototype.redrawPath = function(c, x, y, w, h)
	{
		var s = Math.max(0, Math.min(w, Math.min(h, parseFloat(mxUtils.getValue(this.style, 'size', this.size)))) / 2);
		var arcSize = mxUtils.getValue(this.style, mxConstants.STYLE_ARCSIZE, mxConstants.LINE_ARCSIZE) / 2;
		// this.addPoints(c, [new mxPoint(s, 0), new mxPoint(w - s, 0), new mxPoint(w, s), new mxPoint(w, h), new mxPoint(0, h), new mxPoint(0, s)],
		this.addPoints(c, [new mxPoint(0, 0), new mxPoint(w, 0), new mxPoint(w, h - s), new mxPoint(w - s, h), new mxPoint(s, h), new mxPoint(0, h - s)],
				this.isRounded, arcSize, true);
		c.end();
	};

	mxCellRenderer.registerShape('cycle_end', CycleEndShape);
	...
	mxCellRenderer.registerShape('switch', mxRhombus);
	...
	if (typeof mxVertexHandler !== 'undefined')
	{
		...
		var handleFactory = {
			...
			'switch': createArcHandleFunction(),
			...
			'cycle_begin': createCubeHandleFunction(0.5, CycleBeginShape.prototype.size, true),
			'cycle_end': createCubeHandleFunction(0.5, CycleEndShape.prototype.size, true),
	...
	CycleBeginShape.prototype.constraints = mxRectangleShape.prototype.constraints;
	CycleEndShape.prototype.constraints = mxRectangleShape.prototype.constraints;
